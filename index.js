...
const init = async()=> {
  ...
  const [jo, bow, john, bill, cathy, blackSeedDounut, bodyScrub, sandlewoodCandle, honey] = await Promise.all([
  ...
  const carts = await Promise.all([
    ...
    createCart({ customer_id: john.id, product_id: bodyScrub.id}),
  ]);

  console.log(await fetchCart(jo.id));

  await deleteCart(carts[0].id);
  console.log(await fetchCart(jo.id));
